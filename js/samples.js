...
(function (global) {
  ...
  const SAMPLES = [
    ...
    /* ══════════ 0. ミニマム：YouTubeで動画を順に開く ══════════ */
    {
      ...
      desc: '一番小さな練習用フロー。検索結果に並んだ動画を上から10本、開いて→5秒ながめて→戻る、を繰り返すだけ。▶実行を押すとブラウザが立ち上がり、画面がバーッと切り替わっていくのを目で見られます。ログイン不要なので、まずはこれで動きを確認してください。',
      meta: 'ノード6個 / まずはこれ',
      graph: {
        name: 'ミニマム：YouTubeの動画を順に開く',
        nodes: [
          { id: 's1', type: 'start', title: '開始：YouTubeで検索', x: 60, y: 100,
            data: { url: 'https://www.youtube.com/results?search_query=lofi',
                    base_dir: 'output', headless: false, connect_mode: 'own',
                    use_session: false, polite_wait: 0.5,
                    on_error: 'screenshot_continue',
                    viewport_w: 1440, viewport_h: 900 } },
          { id: 'w1', type: 'wait', title: '検索結果が出るまで待つ', x: 340, y: 100,
            data: { mode: 'visible', target: t('a#video-title, a.ytLockupMetadataViewModelTitle'), timeout: 60 } },
          { id: 'lp', type: 'loop', title: '上から10本くり返す', x: 620, y: 80,
            data: { mode: 'elements', elements_target: t('a#video-title, a.ytLockupMetadataViewModelTitle'),
                    limit: 10, continue_on_error: true } },
          { id: 'ck', type: 'click', title: '{{index}}本目の動画を開く', x: 900, y: 60,
            data: { target: { strategy: 'css', selector: 'a#video-title, a.ytLockupMetadataViewModelTitle', index: 'loop' },
                    wait_after: 'time', wait_ms: 1500 } },
          { id: 'w2', type: 'wait', title: '5秒ながめる', x: 1180, y: 80,
            data: { mode: 'time', ms: 5000 } },
          { id: 'bk', type: 'goto', title: '検索結果へ戻る', x: 1460, y: 80,
    ...
    /* ══════════ 0.5 ログインが要るサイト（自分のChromeにつなぐ） ══════════ */
    {
      key: 'youtube-liked',
      icon: '🔐',
      title: 'ログインが要るサイトを自動化する',
      desc: 'YouTubeの「高く評価した動画」を順に開く例。Google・Microsoftなどは自動操作ブラウザからのログインを拒否するため、▶実行画面の「🌐 ログイン用ブラウザを開く」で普段のChromeを開き、そこでログインしてから実行します。会社の業務サイトでも同じやり方が使えます。',
      meta: 'ノード6個 / ログイン対応',
      graph: {
        name: 'ログインが要るサイトの例（自分のChromeにつなぐ）',
        nodes: [
          { id: 's1', type: 'start', title: '開始：自分のChromeにつなぐ', x: 60, y: 100,
            data: { url: 'https://www.youtube.com/playlist?list=LL',
                    base_dir: 'output', connect_mode: 'cdp',
                    cdp_url: 'http://127.0.0.1:9222', polite_wait: 0.5,
                    on_error: 'screenshot_continue' } },
          { id: 'w1', type: 'wait', title: '一覧が出るまで待つ', x: 340, y: 100,
            data: { mode: 'visible', target: t('a.ytLockupMetadataViewModelTitle, ytd-playlist-video-renderer a#video-title, a#video-title'),
                    timeout: 60 } },
          { id: 'lp', type: 'loop', title: '上から10本くり返す', x: 620, y: 80,
            data: { mode: 'elements',
                    elements_target: t('a.ytLockupMetadataViewModelTitle, ytd-playlist-video-renderer a#video-title, a#video-title'),
                    limit: 10, continue_on_error: true } },
          { id: 'ck', type: 'click', title: '{{index}}本目を開く', x: 900, y: 60,
            data: { target: { strategy: 'css',
                              selector: 'a.ytLockupMetadataViewModelTitle, ytd-playlist-video-renderer a#video-title, a#video-title',
                              index: 'loop' },
                    wait_after: 'time', wait_ms: 1500 } },
          { id: 'w2', type: 'wait', title: '5秒ながめる', x: 1180, y: 80,
            data: { mode: 'time', ms: 5000 } },
